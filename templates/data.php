...
function init() {
	...
	autocomplete("#whatsapp", people, ',');
	autocomplete("#met", people, '+');
	autocomplete("#phone", people, ',');
	autocomplete("#fb", people, ',');
...
<div class="next-holder"><a class="next next-day  with-icon" href="?date=<?php echo date('Y-m-d', strtotime($date) + (60*60*24)); ?>">Next Day(<?php echo date('dS M', strtotime($date) + (60*60*24)); ?>)</a></div>
</div>
<br />
	...
	if($all_connections) {
		...
		foreach($all_connections as $con) {
			...
			foreach($all_people_connections as $pep_con) {
				...
				$all_people[] = '<a href="person.php?id='.$pep_con['person_id'].'">'. ($person['nickname'] | $person['name']) .'</a>';
			}
			
			if($all_people)  print "<li>".implode(', ', $all_people)." <a href='ajax/delete_connection.php?connection_id=$con[id]' class='ajaxify delete icon'>Delete</a> <a href='#' class='popup edit icon'>Details</a></li>";
		}
		print "</ul>";
	}
}